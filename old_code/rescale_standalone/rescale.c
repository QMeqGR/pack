...
double ca_chrg_1=0,ca_chrg_2=0;
...
int main(int argc, char *argv[])
{
  ...
  if ( num_vert==1 ) {
    /* allocate space for sphere structures */
    sphr = (struct sphere *)malloc( num_an * sizeof(struct sphere) );
    if ( !sphr ) {
      printf("%s\n","Not enough memory for spheres.\n");
      exit(0);
    }
  }
  ...
  rescale_anions( num_vert, tetr, octa, dimr, sphr, num_an, &cell, cell_chng, ang_chng);
		    ...
		    )
{
  ...
  if ( n_type_ca == 1 ) {
    errval = fscanf(getfile," %d %lf %lf %d ",&num_ca_1,&ca_chrg_1,Rcat_1,Zcat_1);
    if ( errval != 4 ) { printf("Error reading input file: loc. 4 errval=%d.\n",errval); exit(0); }
    num_ca = num_ca_1;
  } else {
    errval = fscanf(getfile," %d %lf %lf %d %d %lf %lf %d ",
  ...
  if ( debug > 3 ){
    ...
    printf("* get parms: ca_chrg_1   = %f (type is int, 1.0, 2.0, etc. will cause err reading subsequent values)\n",ca_chrg_1);
    printf("* get parms: Rcat_1      = %f\n",*Rcat_1);
    printf("* get parms: Zcat_1      = %d\n",*Zcat_1);
    printf("* get parms: num_ca_2    = %d\n",num_ca_2);
    printf("* get parms: ca_chrg_2   = %f\n",ca_chrg_2);
  ...
  for(i=0;i<num_an;i++) {
    for(j=0;j<num_vert;j++){
      if ( num_vert==5 ) fscanf(getfile," %lf %lf %lf ",
				&tetr[i].v[j].x,
				&tetr[i].v[j].y,
				&tetr[i].v[j].z);
      if ( num_vert==7 ) fscanf(getfile," %lf %lf %lf ",
				&octa[i].v[j].x,
				&octa[i].v[j].y,
				&octa[i].v[j].z);
      if ( num_vert==2 ) fscanf(getfile," %lf %lf %lf ",
				&dimr[i].v[j].x,
				&dimr[i].v[j].y,
				&dimr[i].v[j].z);
      if ( num_vert==1 ) fscanf(getfile," %lf %lf %lf ",
				&sphr[i].v[j].x,
				&sphr[i].v[j].y,
				&sphr[i].v[j].z);
    }
